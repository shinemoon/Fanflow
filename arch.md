...
- **剪贴板URL恢复**: 从剪贴板HTML中提取图片原始URL，实现智能恢复
- **缩略图预览**: 为草稿图片生成并保存缩略图，提供视觉参考
- **存储优化**: 轻量化存储策略，避免localStorage限制，多级错误处理
- **状态管理**: 完整的回复/转发信息保存与清理
- **错误处理**: 完善的异常捕获和降级机制
- **剪贴板增强**: 支持直接粘贴网络图片，智能提取原始URL
...
```javascript
{
    ...
    imageData: {
        ...
        lastModified: 时间戳,
        filePath: "文件路径或URL",
        sourceUrl: "原始URL"(网络图片),
        isPastedFromClipboard: true/false,
        thumbnail: { // 缩略图预览
            dataUrl: "base64数据",
            width: 150,
            height: 100
...
### 存储策略
- **优先级**: 文本内容 > Reply信息 > 图片基本信息 > 缩略图 > 其他数据
- **持续性保证**: 多重数据缓存机制，确保图片信息不丢失
- **存储限制**: 避免超出localStorage 5-10MB限制，智能降级
...
- **持续性保证**: 多重数据缓存和file input状态同步，确保图片草稿不丢失
- **轻量存储**: 图片不保存Base64，只保存URL和基本信息，但包含缩略图预览
- **状态同步**: 文本变化时自动清理reply信息，图片状态完全同步
